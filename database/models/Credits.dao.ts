...
export class CreditsDAO {
  ...
  /**
   * Create user credits
   * Note: This is typically called by the database trigger, not manually
   */
  static async create(userId: string, credits: number = 10): Promise<UserCredits> {
    const now = new Date().toISOString();

    const [result] = await db
      .insert(userCredits)
      .values({
        id: userId, // id IS the user_id in production
        credits,
        updatedAt: now,
      })
      .returning();
