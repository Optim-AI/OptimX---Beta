...
import { campaigns, recommendations } from '@/database/schema';
import { eq, and, desc } from 'drizzle-orm';

// Type inference from Drizzle schema
type Campaign = typeof campaigns.$inferSelect;
type CampaignInsert = typeof campaigns.$inferInsert;
type CampaignWithRecommendations = Campaign & {
...
export class CampaignDAO {
   ...
   * Find campaign by ID
   */
  static async findById(id: string): Promise<CampaignWithRecommendations | null> {
    ...
    const recs = await db
      ...
      .where(eq(recommendations.campaignId, id));
   ...
   */
  static async listByUser(userId: string): Promise<CampaignWithRecommendations[]> {
    ...
    // Fetch recommendations for each campaign
    const result: CampaignWithRecommendations[] = [];
    for (const campaign of campaignList) {
      const recs = await db
        .select()
        .from(recommendations)
        .where(eq(recommendations.campaignId, campaign.id));
