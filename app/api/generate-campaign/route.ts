...
const NANO_API_KEY = process.env.NANO_API_KEY || "AIzaSyADppFic4BIjCi1EoeyBUwjV3HK8IXLPG8";
...
const SUPPORTED_IMAGE_TYPES = [
  'image/jpeg',
  'image/jpg', // Note: We accept this but normalize to image/jpeg for Gemini
  'image/png',
  'image/gif',
  'image/webp',
];
...
function isSupportedImageType(mimeType: string): boolean {
  ...
  return SUPPORTED_IMAGE_TYPES.some(supported => normalized === supported || normalized.startsWith(supported));
}

// Normalize MIME type for Gemini API (Gemini doesn't accept "image/jpg", only "image/jpeg")
function normalizeMimeTypeForGemini(mimeType: string): string {
  if (!mimeType) return 'image/png'; // Default fallback
  const normalized = mimeType.toLowerCase().trim();
  
  // Convert "image/jpg" to "image/jpeg" for Gemini compatibility
  if (normalized === 'image/jpg' || normalized.startsWith('image/jpg')) {
    return 'image/jpeg';
  }
  
  // Return normalized type if it's supported, otherwise default to PNG
  if (isSupportedImageType(normalized)) {
    return normalized;
  }
...
function buildPromptFromInputs(body: any) {
  ...
  // CRITICAL: If frontend provides a complete prompt, use it directly (prioritize it)
  if (body.prompt && typeof body.prompt === 'string' && body.prompt.trim()) {
    // Frontend is sending a fully constructed prompt - use it as-is
    return body.prompt;
  }
  
  // Fallback: If description is provided, use it as the base
  if (body.description && typeof body.description === 'string' && body.description.trim()) {
    parts.push(body.description);
  }
  
  if (body.productProvided === true) {
  parts.push(`
CRITICAL INSTRUCTION: The user has provided a REAL product image (labeled "product_image").
You MUST use this EXACT product image as the PRIMARY and ONLY product visual in the creative.
DO NOT generate, recreate, hallucinate, or modify the product image in any way.
DO NOT change the product's shape, colors, logo, proportions, or any visual details.
DO NOT create a new version of the product.
The product image must remain EXACTLY as provided - pixel perfect, unchanged.
ONLY design the background, lighting, typography, text, and layout elements AROUND the product.
The product itself must stay completely unchanged and clearly recognizable as the original image.
If you cannot use the provided product image exactly as-is, do not generate the creative.
  `.trim());
}
  ...
  // Logo guidance - ONLY if logo is provided
  const logoProvided = body.logoProvided === true || body.logoProvided === "true" || !!body.logoDataUrl;
  if (logoProvided) {
    const placement: LogoPlacement | undefined = body.logoPlacement as LogoPlacement | undefined;

    parts.push(
      "CRITICAL: A brand logo has been provided. You MUST include this logo in the generated poster.",
...
export async function POST(request: Request) {
  try {
    ...
    console.log("⚙️ generate-campaign body keys:", Object.keys(body));
console.log(
  "📦 productDataUrl present?",
  typeof body.productDataUrl,
  body.productDataUrl ? body.productDataUrl.slice(0, 50) + "..." : null
);

    // TEMPORARILY DISABLED AUTH - Authentication check bypassed
    ...
    // TEMPORARILY DISABLED - Use a dummy user ID for credits check
    const user = { id: "temp-user-disabled-auth" } as any;

    // credits - TEMPORARILY DISABLED (skip credits check)
    let currentCredits: number | null = 999; // Set to high value to bypass credits check
    
    // Original credits check (commented out):
    ...
    // TEMPORARILY DISABLED - Skip credits validation
  ...
  saveTemp = false,
    ...
    const prompt = buildPromptFromInputs({
      ...body,
      productProvided: Boolean(productDataUrl),
      vision,
    ...
    // Build Gemini parts: text + inline product/logo/ref data
    const parts: any[] = [{ text: prompt }];

    // CRITICAL: Add product image FIRST if provided - this is the main subject
    if (productDataUrl && typeof productDataUrl === "string" && productDataUrl.startsWith("data:")) {
      ...
      if (m) {
        ...
        // Filter out unsupported image types (SVG, ICO, etc.)
        if (!isSupportedImageType(mimeType)) {
          console.warn(`⚠️ Product image has unsupported MIME type: ${mimeType}. Skipping product image.`);
        } else {
          // Normalize MIME type for Gemini (convert image/jpg to image/jpeg)
          const normalizedMimeType = normalizeMimeTypeForGemini(mimeType);
          parts.push({
            inline_data: {
          ...
          console.log("✅ Added product image to Gemini parts");
        }
      } else {
        console.warn("⚠️ productDataUrl format invalid, expected data: URL");
      }
    } else if (productDataUrl) {
      console.warn("⚠️ productDataUrl present but not a data URL:", typeof productDataUrl);
    }
    ...
    if (Array.isArray(refDataUrls) && refDataUrls.length) {
      ...
      for (const d of refDataUrls) {
        ...
        // Filter out unsupported image types (SVG, ICO, etc.)
        if (!isSupportedImageType(mimeType)) {
          console.warn(`⚠️ Reference image has unsupported MIME type: ${mimeType}. Skipping.`);
          continue;
        }
    ...
    if (logoDataUrl && typeof logoDataUrl === "string" && logoDataUrl.startsWith("data:")) {
      ...
      if (m) {
        ...
        // Filter out unsupported image types (SVG, ICO, etc.)
        if (!isSupportedImageType(mimeType)) {
          console.warn(`⚠️ Logo has unsupported MIME type: ${mimeType}. Skipping logo.`);
        } else {
          // Normalize MIME type for Gemini (convert image/jpg to image/jpeg)
          const normalizedMimeType = normalizeMimeTypeForGemini(mimeType);
          parts.push({
          ...
          console.log("✅ Added logo to Gemini parts");
        }
      }
    }

    // CRITICAL: Emphasize theme in the prompt
    if (theme) {
      parts.push({ text: `CRITICAL: The design MUST follow the "${theme}" theme. This theme should drive the visual style, color palette, typography, mood, and overall composition.` });
    }
    parts.push({ text: `Aspect ratio: ${aspectLabel}. Strictly follow this aspect ratio in the design.` });
    ...
    // Check payload size before sending - use a safer approach for large images
    let payloadSizeMB = 0;
    try {
      // Check product image size first (before stringifying entire payload)
      if (productDataUrl && typeof productDataUrl === "string") {
        const productImageSizeMB = productDataUrl.length / 1024 / 1024;
        console.log("🖼️ Product image size:", `${productImageSizeMB.toFixed(2)}MB`);
        
        if (productImageSizeMB > 10) {
          console.warn("⚠️ Product image is very large, this may cause issues");
          return NextResponse.json(
            { ok: false, error: "Product image is too large. Please use a smaller image (under 5MB recommended). The image will be compressed automatically on the frontend." },
            { status: 400 }
          );
        }
      }
      
      // Try to stringify payload with a size limit check
      const payloadStr = JSON.stringify(payload);
      payloadSizeMB = payloadStr.length / 1024 / 1024;
      console.log("📦 Total payload size:", `${payloadSizeMB.toFixed(2)}MB`);
      
      if (payloadSizeMB > 20) {
        console.warn("⚠️ Payload is very large (>20MB), this may cause issues with Gemini API");
        return NextResponse.json(
          { ok: false, error: "Payload is too large. Please use a smaller product image." },
          { status: 400 }
        );
      }
    } catch (e: any) {
      // Stack overflow or other stringify errors
      if (e.message?.includes("Maximum call stack") || e.message?.includes("stack")) {
        console.error("❌ Stack overflow when stringifying payload - image is too large");
        return NextResponse.json(
          { ok: false, error: "Product image is too large and caused a stack overflow. Please use a smaller image (under 2MB recommended)." },
          { status: 400 }
        );
      }
      console.error("❌ Error calculating payload size:", e.message);
      return NextResponse.json(
        { ok: false, error: "Product image is too large. Please use a smaller image." },
        { status: 400 }
      );
    }
    ...
    console.log("📤 Sending request to Gemini:", {
      ...
      hasProductImage: parts.some((p: any) => p.inline_data?.data && parts.indexOf(p) === 1), // Product image is usually second part (after text)
      hasLogo: parts.some((p: any) => p.inline_data?.data),
      payloadSizeMB: payloadSizeMB > 0 ? `${payloadSizeMB.toFixed(2)}MB` : "unknown",
    });

    const createResp = await axios.post(url, payload, { headers }).catch((err) => {
      const r = err.response?.data ?? err.message;
      console.error("❌ Gemini create error:", {
        error: r,
    ...
    // decrement credits - TEMPORARILY DISABLED (skip credit decrement)
    let updatedCredits: number | null = currentCredits; // Keep same credits value
    
    // Original credit decrement (commented out):
