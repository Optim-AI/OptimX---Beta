...
>(({ className, children, style, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      "flex h-10 w-full items-center justify-between rounded-md px-3 py-2 text-sm ring-offset-background focus:outline-none disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1",
      className
    )}
    style={
      {
...
>(({ className, children, style, ...props }, ref) => (
  <SelectPrimitive.Item
    ...
    style={{
      ...
      color: colors.foreground,
      // item focus / active states (hover/focus/data-state) are kept via class names for animations,
      // but their colors can be driven via CSS variables if desired later.
      ...(style as React.CSSProperties),
    }}
    {...props}
  >
    <span className="absolute left-2 flex h-3.5 w-3.5 items-center justify-center" style={{ color: colors.primary }}>
...
>(({ className, style, ...props }, ref) => (
  <SelectPrimitive.Separator
    ...
    style={{ backgroundColor: colors.muted, height: "1px", ...(style as React.CSSProperties) }}
    {...props}
  />
))
