...
>(({ className, inset, children, ...props }, ref) => (
  <ContextMenuPrimitive.SubTrigger
    ref={ref}
    className={cn(
      // layout + animation classes - color-specific classes removed in favor of inline style
      "flex cursor-default select-none items-center rounded-sm px-2 py-1.5 text-sm outline-none",
      inset && "pl-8",
      className
    )}
    // base style for text/background using your color tokens
    style={{
      background: "transparent",
      color: colors.foreground,
      // provide a focus outline using your ring color (keeps visual focus)
...
>(({ className, ...props }, ref) => (
  <ContextMenuPrimitive.SubContent
    ref={ref}
    className={cn(
      "z-50 min-w-[8rem] overflow-hidden rounded-md border p-1 text-sm shadow-md data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2",
      className
    )}
    // use your color tokens for background, text and border
    style={{
      background: colors.popover,
      color: colors.popoverForeground,
      borderColor: colors.border,
      // keep subtle shadow using your shadow token if desired (still CSS string)
