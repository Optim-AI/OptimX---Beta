// pages/api/auth/facebook/getAds.ts
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const fields = ["id","name","status","adset_id","campaign_id","creative"].join(",");
    const normalizedAdAccount = String(adAccountId).replace(/^act_/, "");
    const url = `https://graph.facebook.com/v${VERSION}/${encodeURIComponent(normalizedAdAccount)}/ads?fields=${encodeURIComponent(fields)}&access_token=${encodeURIComponent(userAccessToken)}`;
    ...
    console.error("facebook/getAds error:", err);
