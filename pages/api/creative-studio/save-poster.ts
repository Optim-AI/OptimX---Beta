...
async function getUserFromRequest(req: NextApiRequest): Promise<{ user: any; userId: string } | null> {
  // TEMPORARILY DISABLED: Auth check - return null to indicate no authenticated user
  // This allows the API to work without auth but skip database inserts
  return null;
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Use authenticated user ID or generate a temporary UUID for storage paths
    // Note: We'll skip database insert if no auth, but still allow storage upload
    const userId = auth?.userId || randomUUID();
