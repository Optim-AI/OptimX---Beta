...
const GEMINI_API_KEY = process.env.GEMINI_API_KEY || process.env.GEMINI_VEO_API_KEY || "AIzaSyDcEtzSyPBbVwR_Ygt9mkIUgownDyV9FRs";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // ========================================
    // Step A: Use Gemini 3.1 as "Creative Director Brain"
...
IMPORTANT: shot_plan and storyboard durations must cover 0 to ${durationSecondsClamped} seconds total. final_video_prompt must describe the entire ${durationSecondsClamped}-second film with rich cinematic language. Return ONLY valid JSON; no markdown or extra text.`;
    ...
    // This is the "Mini Gemini Studio" approach - Gemini expands the prompt
    // Using gemini-2.5-flash with v1beta REST API
    // Use object format for system_instruction as per REST API docs
    const requestBody = {
      contents: [
        {
          parts: [
    ...
    if (!response.ok) {
      ...
      console.error("Request URL:", `${GEMINI_BASE_URL}/models/gemini-1.5-pro:generateContent`);
    ...
    try {
      ...
      console.log("JSON preview:", cleanedText.substring(0, 200));
        ...
        storyboard: [
          {
            ...
            emotion: "Desire",
            motion_style: "Smooth, cinematic",
            voiceover_script: voiceoverText || "",
          },
        ],
        visual_style_guide: {
